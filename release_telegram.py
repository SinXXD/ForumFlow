...
def send_files(
    ...
    total = len(batches)
    caption = f"ForumFlow v{version} - 安装包"[:1024]
...
def main() -> int:
    ...
    if is_prerelease:
        title_html = f"🧪 <b>ForumFlow v{html_escape(version)}（预发布）</b> #beta"
    else:
        title_html = f"🚀 <b>ForumFlow v{html_escape(version)}</b> #stable"
